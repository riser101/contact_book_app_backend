...
class TestViews(TestBase):
	...
	def test_create_view(self):
		with self.client:
			...
			# no name param error
			response = self.client.post('/create', data={'contact_number':9889887899, 'email':'test3@gmail.com'})
			self.assertEqual(response.status_code, 400)
			# no contact param error
			response = self.client.post('/create', data={'name':'test_contact_3', 'email':'test3@gmail.com'})
			self.assertEqual(response.status_code, 400)			
			# no email param error
			response = self.client.post('/create', data={'name':'test_contact_3',
		 		'contact_number':9889887899})
			self.assertEqual(response.status_code, 400)			
	...
	def test_edit_view(self):
		with self.client:
			...
			response = self.client.put('/edit', data={'contact_id':1, 'name':'updated_name',
				 'contact_number':9887887677, 'email':'edit_test@gmail.com'})
			self.assertEqual(response.status_code, 200)
			...
			response = self.client.put('/edit', data={'contact_id':1, 'contact_number':9887887677, 
